...
import com.liujs.library.view.refresh.util.BGARefreshScrollingUtil;
                ...
                if (!mIsLoadingMore && (mCurrentRefreshStatus != RefreshStatus.REFRESHING)) {
                    ...
                    if (Math.abs(event.getRawX() - mInterceptTouchDownX) < Math.abs(interceptTouchMoveDistanceY) && mRefreshHeaderView != null) {
                        if ((interceptTouchMoveDistanceY > mTouchSlop && shouldHandleRefresh()) || (interceptTouchMoveDistanceY < -mTouchSlop && shouldHandleLoadingMore())
                                || (interceptTouchMoveDistanceY < -mTouchSlop && !isWholeHeaderViewCompleteInvisible())
                                || (interceptTouchMoveDistanceY > mTouchSlop && shouldInterceptToMoveCustomHeaderViewDown())){
                            isintercept = true;
                       //     Log.d("refreshLayout","isintercept = true;");
                            onTouchEvent(event);
                        }
                    }
                } //Log.d("refreshLayout","mCurrentRefreshStatus== "+RefreshStatus.RELEASE_REFRESH);
                break;
